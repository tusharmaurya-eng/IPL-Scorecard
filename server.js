...
function buildScorecardUrl(matchCenterUrl) {
  ...
  if (!u.startsWith('/')) u = '/' + u;
  u = u
    .replace('/cricket/matchcentre/', '/cricket/scorecard/')
    .replace('/cricket/current/', '/cricket/scorecard/');
...
function buildCurrentUrl(matchCenterUrl) {
  if (!matchCenterUrl) return null;
  let u = String(matchCenterUrl).trim();
  if (!u || u.includes('google.com/calendar')) return null;
  u = u.split('#')[0].replace(/\/+$/, '');
  ...
  if (!u.startsWith('/')) u = '/' + u;
  // Sportsstar/The Hindu uses /api/cricket/current/... for the current-over/live strip.
  u = u
    .replace('/cricket/matchcentre/', '/cricket/current/')
    .replace('/cricket/scorecard/', '/cricket/current/');
  if (u.startsWith('/cricket/current/')) {
    return `https://livescoreapi.thehindu.com/api${u}`;
  }
  if (u.startsWith('/current/')) {
    return `${BASE}${u}`;
  }
  return null;
}
...
function extractCurrentOver(rawScorecard, rawCurrent) {
  const roots = [rawCurrent, rawCurrent?.data, rawScorecard, rawScorecard?.data].filter(Boolean);

  // Best source: current timeline [{overs, runs:[{run, ball}]}]
  ...
  // Second source: bowler.thisover.runs from current.
  const bowlerArrays = [];
  for (const root of roots) {
    (function walk(o){
      if (!o || typeof o !== 'object') return;
...
function extractCurrentOverWithSource(rawCurrent) {
  // The scorecard API does not contain delivery-by-delivery current-over balls.
  // Current-over balls must come from the CURRENT companion API:
  // /api/cricket/current/... gives timeline[].runs[], bowlers[].thisover.runs, or commentary[].type + over_num.
  const balls = extractCurrentOver(null, rawCurrent);
  return {
    balls,
    source: balls.length ? 'current' : 'none'
  };
}
...
function normalizeScorecard(raw, fixtureMatch, rawCurrent = null) {
  ...
  const overInfo = extractCurrentOverWithSource(rawCurrent);
...
function enrichFixtureMatchWithScorecard(fixtureMatch, rawScorecard, rawCurrent = null) {
  const decoded = normalizeScorecard(rawScorecard, fixtureMatch, rawCurrent);
...
async function refresh() {
  const errors = {};
  let fixtures = null, points = cache.points, players = cache.players, rawScorecard = null, rawCurrent = null, liveScore = null, liveUrl = null, currentUrl = null;
  ...
  try {
    ...
    currentUrl = buildCurrentUrl(cm?.match_center_url);
    if (liveUrl) {
      rawScorecard = await fetchJson(liveUrl);
      if (currentUrl) {
        try { rawCurrent = await fetchJson(currentUrl, 7000); }
        catch (e) { errors.current = `Current-over source failed: ${e.message}`; }
      }
      liveScore = normalizeScorecard(rawScorecard, cm, rawCurrent);
      if (fixtures?.current_matches?.length && liveScore) {
        fixtures = { ...fixtures, current_matches: fixtures.current_matches.map((m,i) => {
          ...
          return isSame ? enrichFixtureMatchWithScorecard(m, rawScorecard, rawCurrent) : m;
        }) };
      }
    } else if (cm) {
  ...
  cache = { fixtures, points, players, rawScorecard, rawCurrent, liveScore, liveUrl, currentUrl, updatedAt: new Date().toISOString(), errors };
  return cache;
}
...
app.get('/api/status', async (req,res)=>{
  ...
  res.json({ ok: !cache.errors || Object.keys(cache.errors).length === 0 || !!cache.fixtures, updatedAt: cache.updatedAt, liveUrl: cache.liveUrl, currentUrl: cache.currentUrl, errors: cache.errors, endpoints: URLS });
});
...
app.get('/api/live-score', async (req,res)=>{
  ...
  res.json({ ok:true, liveUrl:cache.liveUrl, currentUrl:cache.currentUrl, updatedAt:cache.updatedAt, normalized:cache.liveScore.normalized, rawDecoded: { currentBatters: cache.liveScore.currentBatters, currentBowler: cache.liveScore.currentBowler, innings: cache.liveScore.normalized.innings, currentOver: cache.liveScore.normalized.currentOver, currentOverSource: cache.liveScore.normalized.currentOverSource } });
});
...
app.get('/api/raw-current', async (req,res)=>{
  if (!cache.rawCurrent) await refresh().catch(()=>{});
  res.json(cache.rawCurrent || {api_error:true, message:'No raw current ball-by-ball data yet', currentUrl:cache.currentUrl, errors:cache.errors});
});
